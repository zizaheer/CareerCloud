    ...
    public class CompanyLocationLogic : BaseLogic<CompanyLocationPoco>
    {
        ...
        protected override void Verify(CompanyLocationPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(500, $"Country code cannot be null or empty."));
                }
                if (string.IsNullOrEmpty(poco.Province))
                {
                    exceptions.Add(new ValidationException(501, $"Province cannot be null or empty."));
                }
                if (string.IsNullOrEmpty(poco.Street))
                {
                    exceptions.Add(new ValidationException(502, $"Street cannot be null or empty."));
                }
                if (string.IsNullOrEmpty(poco.City))
                {
                    exceptions.Add(new ValidationException(503, $"City cannot be null or empty."));
                }
                if (string.IsNullOrEmpty(poco.PostalCode))
                {
                    exceptions.Add(new ValidationException(504, $"Postal code cannot be null or empty."));
                }
            }
